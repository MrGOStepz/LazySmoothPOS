...
public interface TableInfoRepository extends JpaRepository<TableInfo, Long> {
    ...
    @Transactional
    @Modifying
    @Query("UPDATE table_info ti SET ti.name = :name, ti.status = :status, ti.order = :order WHERE ti.name = :name")
    void updateTableInfo(@Param("name")String name, @Param("status")String status, @Param("order")Integer order);
